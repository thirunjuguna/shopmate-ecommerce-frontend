...
const actions = [
  { type: FETCH_PRODUCTS, action: productListActions.fetchProducts, expected: true },
  {
    type: FETCH_PRODUCTS_FAILURE, action: productListActions.fetchProductsFailure, expected: false,
  },
  {
    type: FETCH_PRODUCTS_SUCCESS, action: productListActions.fetchProductsSuccess, expected: false,
  },
];

describe('productList actions', () => {
  actions.map(action => it(`should dispatch ${action.type}`, () => {
